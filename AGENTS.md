...
- A pushed branch, an accepted review report, or completed manual testing does not by itself unlock a dependent Story. The prerequisite is merged only when its immutable required full commit SHA is an ancestor of `main` and `main` matches `origin/main`.
- Before starting a dependent Story, fetch `origin` and verify each named prerequisite with `git merge-base --is-ancestor <required-full-commit-sha> main`. A branch name may be recorded only as a locator for resolving and cross-checking that immutable SHA; never use a movable or deleted branch tip as the downstream unlock fact. If the ancestry or synchronization check fails, stop before creating a branch or modifying files and return to the missing review / merge gate.
- A status-document disagreement does not by itself block planning validation, Story branch creation, or the current dependent Story when all of the following are true: Git proves the prerequisite is merged; the disagreement is only stale status, completion marking, merge identity, or other bookkeeping text; the correction does not change or conceal product requirements, Architecture, ownership, scope, acceptance criteria, evidence requirements, or implementation readiness; the current Story explicitly lists the exact affected documents, old status, new status, and validation method; and the correction is a direct causal ripple of the current Story that does not rewrite historical evidence. Make that minimum correction in the same reviewable candidate.
- If a disagreement changes, conceals, or cannot uniquely determine product requirements, Architecture, ownership, scope, acceptance criteria, evidence requirements, implementation readiness, or prerequisite identity, resolve it through an independent, scoped, reviewable planning/docs gate before dependent implementation begins.
- Never choose a status version merely for convenience. Git ancestry determines the merge fact, and documentation may be synchronized only to that proven fact.
